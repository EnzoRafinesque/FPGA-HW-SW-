...
namespace eval debug_profile {
  ...
  proc add_debug_profile {fp hw_platform_info config_info debug_profile_info} {
    ...
    if {$dpa_dict != {} || $deadlock_detection != "disable"} {
  ...
  proc get_dpa_dictionary_memory {fp profile_info dpa_dict} {
    ...
    if {$mss_ap == {}} {
      send_msg_id "DPA-102" "WARNING" "Memory profiling is only supported on platforms with a memory subsystem."
      return $dpa_dict
    }
    ...
    foreach memory $memories {
      ...
      if {[string first "HBM" $memory] >= 0} {
        if {!$reportedHBM} {
          send_msg_id "DPA-101" "WARNING" "Profiling of HBM is not supported."
      ...
      for { set banknum $firstIndex } { $banknum <= $lastIndex } { incr banknum } {
        ...
        if {!$foundSpace} {
          send_msg_id "DPA-100" "WARNING" "Memory profiling was requested on $memoryName\[$banknum] but no kernel connection was found."
          continue
        }
